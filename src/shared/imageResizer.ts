class Resizer {
    ...
    static resizeAndRotateImage(
        ...
        if (rotation && (rotation === 90 || rotation === 270)) {
            canvas.width = <number>newHeightWidth.height;
            canvas.height = <number>newHeightWidth.width;
        } else {
            // @ts-ignore
            canvas.width = newHeightWidth.width;
            canvas.height = <number>newHeightWidth.height;
        }
    ...
    static createResizedImage(
        ...
        responseUriFunc: (arg0: string | Blob) => void,
        outputType = "base64",
        minWidth = null,
        minHeight = null,
    ) {
